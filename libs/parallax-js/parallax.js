...
var Parallax = function () {
  ...
  _createClass(Parallax, [{
    ...
    value: function setPosition(element, x, y) {
      if (element.classList.contains('about__term')) {
        x = x.toFixed(this.precision) + 'px';
        y = -1 * y.toFixed(this.precision) + 'px';
      }
      else {
        x = -1 * x.toFixed(this.precision) + 'px';
        y = y.toFixed(this.precision) + 'px';
      }
      

      var offsetX = parseFloat(element.getAttribute('data-offset-x')),
          offsetY = parseFloat(element.getAttribute('data-offset-y'));   

      var X = parseFloat(x) + offsetX,
      ...
      if (this.transform3DSupport) {
        var str = 'translate3d(' + (X) + ',' + (Y) + ', 0)';
        if (element.classList.contains('about__picture')) {
          str += ' rotateY(-20deg)';
        }

        helpers.css(element, 'transform', str);
